...
export function ProductApplicationModal({
  ...
  const { data: technicalFiles, isLoading: isLoadingFiles } = useAdminProductTechnicalFiles(productId);
                ...
                <>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <p className="font-medium">
                          {product && format(new Date(product.created_at), "MMMM d, yyyy")}
                        </p>
                      </div>
                      <div>
                        <p className="text-sm text-muted-foreground">Last Updated</p>
                        <p className="font-medium">
                          {product && format(new Date(product.updated_at), "MMMM d, yyyy")}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="bg-muted/50 rounded-lg p-4 border space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="font-medium">{product?.manufacturers?.name}</p>
                      </div>
                      <div>
                        <p className="text-sm text-muted-foreground">Country</p>
                        <p className="font-medium">{product?.manufacturers?.country}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <Accordion type="single" collapsible>
                    <AccordionItem value="directives">
                      ...
                      <AccordionContent>
                        {product?.directives && product.directives.length > 0 ? (
                          <div className="space-y-2">
                            {product.directives.map((directive, index) => (
                          ...
                          <p className="text-muted-foreground">No directives specified</p>
                        )}
                      </AccordionContent>
                    </AccordionItem>
                    ...
                    <AccordionItem value="regulations">
                      ...
                      <AccordionContent>
                        {product?.regulations && product.regulations.length > 0 ? (
                          <div className="space-y-2">
                            {product.regulations.map((regulation, index) => (
                              <div key={index} className="bg-background p-2 rounded border">
                                {regulation}
                              </div>
                            ))}
            ...
            <TabsContent value="documents" className="mt-6">
              {isLoadingFiles ? (
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-6 text-center">
                  ...
                  <h3 className="font-medium text-amber-800">No Technical Files Uploaded</h3>
                  <p className="text-amber-700 mt-2">
                    The customer has not uploaded any technical files for this product yet.
                  </p>
                </div>
              ) : (
                  ...
                  {technicalFiles?.map((file) => (
                    ...
                    >
                      <div className="flex justify-between mb-2">
                        <h4 className="font-medium">{formatFileType(file.file_type)}</h4>
                        {file.not_required ? (
                      ...
                      {file.not_required ? (
                        <p className="text-sm text-muted-foreground">
                          Reason: {file.not_required_reason || "No reason provided"}
                        </p>
                      ) : file.file_url ? (
                      ...
                      <p className="text-xs text-muted-foreground mt-2">
                        Last updated: {format(new Date(file.updated_at), "MMMM d, yyyy")}
                      </p>
                    </div>
                  ))}
                </div>
