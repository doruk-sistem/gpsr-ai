      ...
      try {
        ...
        const usersWithCounts = await Promise.all(
          filteredUsers.map(async (user) => {
            ...
            const { count: manufacturerCount, error: manufacturerError } =
              await supabase
                .from("manufacturers")
                .select("*", { count: "exact", head: true })
                .eq("user_id", user.id);
        ...
        if (params.subscriptionStatus) {
          if (params.subscriptionStatus === "none") {
            ...
            finalUsers = finalUsers.filter(
              (user) => user.subscription?.status === params.subscriptionStatus
            );
          }
        }
        ...
        if (params.dateFrom) {
          finalUsers = finalUsers.filter(
            (user) => new Date(user.created_at) >= params.dateFrom!
          );
        }
        ...
        if (params.dateTo) {
          ...
          finalUsers = finalUsers.filter(
            (user) => new Date(user.created_at) <= dateTo
          );
        }
