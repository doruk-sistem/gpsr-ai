...
export const useInvoices = (params: FetchInvoicesParams = {}) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const actualSortBy = sortBy === "created_at" ? "order_date" : sortBy;
      ...
      if (search && search.trim() !== "") {
        query = query.or(
          `payment_intent_id.ilike.%${search}%,checkout_session_id.ilike.%${search}%`
        );
      }
      ...
      // Apply sorting
      query = query.order(actualSortBy, { ascending: sortDirection === "asc" });
      ...
      const invoices: Invoice[] = data
        ...
        .map((order: any) => ({
          id: order.order_id,
          invoice_number:
            order.payment_intent_id?.substring(3, 11).toUpperCase() ||
            "INV-" + Math.random().toString(36).substring(2, 10).toUpperCase(),
          amount: order.amount_total ? order.amount_total / 100 : 0,
          ...
          pdf_url: `/api/invoices/${
            order.payment_intent_id || order.order_id
          }/pdf`, // Mock URL for PDF
        }));
